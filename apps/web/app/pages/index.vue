  ...
  <div class="space-y-10">
    ...
    <section v-if="stats" class="p-6 rounded-lg border border-gray-800">
      ...
      <div class="grid grid-cols-4 gap-4 mb-4">
        <div class="text-center p-3 rounded bg-gray-900/50">
          ...
          <div class="text-xs text-gray-500">{{ t('home.repos') }}</div>
        </div>
        <div class="text-center p-3 rounded bg-gray-900/50">
          <div class="text-2xl font-bold text-yellow-400">{{ stats.totalStars }}</div>
          <div class="text-xs text-gray-500">{{ t('home.stars') }}</div>
        </div>
        <div class="text-center p-3 rounded bg-gray-900/50">
          <div class="text-2xl font-bold text-white">{{ stats.followers }}</div>
          <div class="text-xs text-gray-500">{{ t('home.followers') }}</div>
        </div>
        <div class="text-center p-3 rounded bg-gray-900/50">
          <div class="text-2xl font-bold text-white">{{ stats.following }}</div>
          <div class="text-xs text-gray-500">{{ t('home.following') }}</div>
        </div>
      </div>
      ...
      <div v-if="stats.recentRepos?.length">
        <h4 class="text-sm font-semibold text-gray-400 mb-2">{{ t('home.recentRepos') }}</h4>
        <div class="space-y-2">
          <a
    ...
    <section class="grid grid-cols-2 md:grid-cols-3 gap-4">
      <NuxtLink to="/chat" class="p-6 rounded-lg border border-gray-800 hover:border-blue-500 transition-colors group">
        <h3 class="text-lg font-semibold group-hover:text-blue-400 transition-colors">{{ t('nav.chat') }}</h3>
        <p class="text-sm text-gray-500 mt-1">{{ t('home.chat.desc') }}</p>
      </NuxtLink>
      <NuxtLink to="/prompts" class="p-6 rounded-lg border border-gray-800 hover:border-purple-500 transition-colors group">
        <h3 class="text-lg font-semibold group-hover:text-purple-400 transition-colors">{{ t('nav.prompts') }}</h3>
        <p class="text-sm text-gray-500 mt-1">{{ t('home.prompts.desc') }}</p>
      </NuxtLink>
      <NuxtLink to="/vault" class="p-6 rounded-lg border border-gray-800 hover:border-green-500 transition-colors group">
        <h3 class="text-lg font-semibold group-hover:text-green-400 transition-colors">{{ t('nav.vault') }}</h3>
        <p class="text-sm text-gray-500 mt-1">{{ t('home.vault.desc') }}</p>
      </NuxtLink>
      <NuxtLink to="/blog" class="p-6 rounded-lg border border-gray-800 hover:border-yellow-500 transition-colors group">
        <h3 class="text-lg font-semibold group-hover:text-yellow-400 transition-colors">{{ t('nav.blog') }}</h3>
        <p class="text-sm text-gray-500 mt-1">{{ t('home.blog.desc') }}</p>
      </NuxtLink>
      <NuxtLink to="/showcase" class="p-6 rounded-lg border border-gray-800 hover:border-pink-500 transition-colors group">
        <h3 class="text-lg font-semibold group-hover:text-pink-400 transition-colors">{{ t('nav.showcase') }}</h3>
        <p class="text-sm text-gray-500 mt-1">{{ t('home.showcase.desc') }}</p>
      </NuxtLink>
      <NuxtLink to="/blog/new" class="p-6 rounded-lg border border-gray-800 hover:border-blue-500/30 transition-colors group">
        <h3 class="text-lg font-semibold group-hover:text-gray-300 transition-colors">{{ t('home.newPost') }}</h3>
        <p class="text-sm text-gray-500 mt-1">Markdown</p>
      </NuxtLink>
    </section>
  </div>
</template>
...
onMounted(async () => {
  try {
    ...
    error.value = t('home.statsUnavailable');
  }
});
</script>
